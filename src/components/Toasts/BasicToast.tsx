...
function BasicToast(props: ModalProps) {
  if (!props.isOpen) {
    return null;
  } else {
    setTimeout(() => {
  ...
  return (
    <div
      className={`text-sm text-white rounded-lg fixed right-1 top-14 z-50 ${
        props.backgroundClass === "" ? "bg-green-500" : props.backgroundClass
      }`}
      role="alert"
    >
      <div className="flex p-4 justify-between">
        <p className="pb-1 pr-[1rem]">{props.text}</p>
